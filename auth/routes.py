...
def register():
    """Create a new user. Body: { username, password, first_name, last_name, phone } (required)."""
    ...
    try:
        user = create_user(
            username, password,
            first_name=first_name, last_name=last_name, phone=phone, email=None,
        )
...
def update_me():
    """Update current user's profile fields (first/last name). Phone is not editable."""
    if not getattr(g, "current_user", None):
        return jsonify({"error": "Not authenticated"}), 401
    data = request.get_json() or {}
    first_name = (data.get("first_name") or "").strip() or None
    ...
    if not last_name:
        return jsonify({"error": "Last name is required"}), 400
    user = update_user_name(g.current_user["id"], first_name=first_name, last_name=last_name)
    if not user:
        return jsonify({"error": "User not found"}), 404
    return jsonify(_user_safe(user)), 200
